    ...
    def about(self, user_filter, only_main):
        ...
        if only_main:
            base_query = base_query.filter(
                character=models.F("user__profile__main_character")
            )

        user_characters = base_query.values("character__character_id")

        user_alliances = base_query.filter(character__alliance_id__isnull=False).values(
            "character__alliance_id"
        )

        user_corps = base_query.values("character__corporation_id")

        user_factions = base_query.filter(character__faction_id__isnull=False).values(
            "character__faction_id"
        )

        return self.filter(
            models.Q(
                contact_id__in=user_characters,
                contact_type=Contact.ContactTypeOptions.CHARACTER,
            )
            | models.Q(
                contact_id__in=user_corps,
                contact_type=Contact.ContactTypeOptions.CORPORATION,
            )
            | models.Q(
                contact_id__in=user_alliances,
                contact_type=Contact.ContactTypeOptions.ALLIANCE,
            )
            | models.Q(
                contact_id__in=user_factions,
        ...
        return self.annotate(
            contact_name_annotation=models.Case(
                models.When(
                    ...
                    then=models.Subquery(
                        EveCharacter.objects.filter(
                            character_id=models.OuterRef("contact_id")
                ...
                models.When(
                    ...
                    then=models.Subquery(
                        EveCorporationInfo.objects.filter(
                            corporation_id=models.OuterRef("contact_id")
                ...
                models.When(
                    ...
                    then=models.Subquery(
                        EveAllianceInfo.objects.filter(
                            alliance_id=models.OuterRef("contact_id")
                ...
                models.When(
                    contact_type=Contact.ContactTypeOptions.FACTION,
                    then=models.Subquery(
                        EveFactionInfo.objects.filter(
                            faction_id=models.OuterRef("contact_id")
                        ).values("faction_name"),
                    ),
                ),
...
class Contact(models.Model):
    ...
    def filter_missing_contact_name(cls, pk_list: list[int]) -> list[int]:
        return list(
            cls.objects.filter(pk__in=pk_list)
            .annotate(
...
class AllianceContactLabel(ContactLabel):
    alliance = models.ForeignKey(
        EveAllianceInfo, on_delete=models.RESTRICT, related_name="contact_labels"
    )
...
class AllianceContact(Contact):
    alliance = models.ForeignKey(
        EveAllianceInfo, on_delete=models.RESTRICT, related_name="contacts"
    )
...
class AllianceToken(ContactToken):
    alliance = models.OneToOneField(
        EveAllianceInfo, on_delete=models.RESTRICT, related_name="+"
    )
    ...
    def visible_for(cls, user: User):
        ...
        return cls.objects.filter(
            alliance__alliance_id__in=CharacterOwnership.objects.filter(
                user=user
...
class CorporationContactLabel(ContactLabel):
    corporation = models.ForeignKey(
        EveCorporationInfo, on_delete=models.RESTRICT, related_name="contact_labels"
    )
...
class CorporationContact(Contact):
    corporation = models.ForeignKey(
        EveCorporationInfo, on_delete=models.RESTRICT, related_name="contacts"
    )

    labels = models.ManyToManyField(
        CorporationContactLabel, blank=True, related_name="contacts"
    )
...
class CorporationToken(ContactToken):
    corporation = models.OneToOneField(
        EveCorporationInfo, on_delete=models.RESTRICT, related_name="+"
    )
    ...
    def visible_for(cls, user: User):
        ...
        return cls.objects.filter(
            corporation__corporation_id__in=CharacterOwnership.objects.filter(
                user=user
...
class StandingFilter(BaseFilter):
    ...
    check_type = models.CharField(
        max_length=3,
        choices=CheckTypeOptions.choices,
        default=CheckTypeOptions.AT_LEAST_ONE_CHARACTER,
    )
    only_main = models.BooleanField(
        default=False, help_text="Only consider the main character of the user"
    )

    corporations = models.ManyToManyField(
        EveCorporationInfo,
        blank=True,
        related_name="corp_standing_filters",
        help_text="The corporations that have set the standings",
    )
    alliances = models.ManyToManyField(
        EveAllianceInfo,
        blank=True,
        related_name="alliance_standing_filters",
        help_text="The alliances that have set the standings",
    )
    ...
    def _standing_lookup(self):
        ...
        if self.comparison == self.ComparisonOptions.GREATER_OR_EQUAL:
            return models.Q(standing__gte=self.standing)
        if self.comparison == self.ComparisonOptions.LESS_THAN:
            return models.Q(standing__lt=self.standing)
        if self.comparison == self.ComparisonOptions.LESS_OR_EQUAL:
            return models.Q(standing__lte=self.standing)
        return models.Q(standing=self.standing)

    def _corp_query(self, user_filter):
        return CorporationContact.objects.about(user_filter, self.only_main).filter(
            self._standing_lookup,
            corporation__in=self.corporations.all(),
        )

    def _alliance_query(self, user_filter):
        return AllianceContact.objects.about(user_filter, self.only_main).filter(
            self._standing_lookup,
    ...
    def audit_filter(self, users):
        ...
        if self.check_type == StandingFilter.CheckTypeOptions.AT_LEAST_ONE_CHARACTER:
            ...
            for user in annotated_query.values("pk", "check"):
                output[user["pk"]] = {
                    "message": "User has a character, corporation, alliance or faction that meets the filter"
                    if user["check"]
                    else "User does not meet the filter",
                    "check": user["check"],
                }
            ...
            for user in annotated_query.values("pk", "check"):
                output[user["pk"]] = {
                    "message": "User has all characters that meet the filter"
                    if user["check"]
                    else "User does not meet the filter",
                    "check": user["check"],
                }
            ...
            annotated_query = users.annotate(
                count_valid=models.Subquery(chars)
            ).annotate(
                total=models.Subquery(
                    base_query.values("user")
                    .annotate(count=models.Count("*"))
                    .values("count"),
                )
            )
