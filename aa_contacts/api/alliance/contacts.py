...
def list_contacts(request, alliance_id: int = Path(...)):
    return common.list_contacts(common.ALLIANCE, alliance_id, request.user)


@router.post("/update", response={200: None, 403: None, 404: None})
def update_contacts(request, alliance_id: int = Path(...)):
    return common.update_contacts(common.ALLIANCE, alliance_id, request.user)
...
def edit_contact(
    ...
    return common.edit_contact(
        common.ALLIANCE, alliance_id, contact_pk, data, request.user
    )
...
def update_server_link(
    ...
    return common.update_server_link(
        common.ALLIANCE, alliance_id, contact_pk, link_pk, data, request.user
    )
...
def delete_server_link(
    ...
    return common.delete_server_link(
        common.ALLIANCE, alliance_id, contact_pk, link_pk, request.user
    )
